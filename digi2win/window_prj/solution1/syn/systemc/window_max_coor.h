...
#ifndef _window_max_coor_H_
#define _window_max_coor_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "window_max_coor_memcore.h"

SC_MODULE(window_max_coor) {
    static const unsigned int DataWidth    = 10;
...
protected:
    ...
    // buffer instances
    window_max_coor_memcore* buffer[BufferCount];
...
public:
    ~window_max_coor();
    SC_CTOR(window_max_coor) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "window_max_coor_memcore_U_";
            name += szTmp;

            buffer[i] = new window_max_coor_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "window_max_coor.trace");
            // ports
