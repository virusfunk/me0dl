...
if {$opt(reset)} {
  open_project -reset window_prj
} else {
  open_project window_prj
}
set_top window
add_files firmware/window.cpp -cflags "-std=c++0x"
add_files -tb window_test.cpp -cflags "-std=c++0x"
...
if {$opt(cosim)} {
  ...
  add_files -tb window_test.cpp -cflags "-std=c++0x -DRTL_SIM"
  set time_start [clock clicks -milliseconds]
  cosim_design -trace_level all
  set time_end [clock clicks -milliseconds]
  puts "INFO:"
  puts [read [open window_prj/solution1/sim/report/window_cosim.rpt r]]
...
if {$opt(vsynth)} {
  ...
  if {[file exist window_prj/solution1/syn/vhdl]} {
    set time_start [clock clicks -milliseconds]
